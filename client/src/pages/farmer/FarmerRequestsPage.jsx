...
export default function FarmerRequestsPage() {
  ...
  const handleCancel = async (id) => {
    if (!window.confirm('Are you sure you want to cancel this request? Any assigned slot will be immediately reallocated to waiting farmers.')) {
      return;
    }
    try {
      setCancellingId(id);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-agri-600 block mb-1">
            Fair Scarcity Tracking
          </span>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
            My Resource Requests
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Track status, priority allocations, waitlist positions, and mathematical scoring factors.
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-3 text-xs font-semibold">
        {['ALL', 'SCHEDULED', 'WAITLIST', 'CONFLICT', 'PENDING', 'CANCELLED'].map((tab) => (
          <button
            key={tab}
      ...
      {loading ? (
        <div className="text-center py-16 text-slate-400 text-sm">
          ...
          Loading your request queue...
        </div>
      ) : filteredRequests.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-400 space-y-3">
          <Tractor className="w-12 h-12 mx-auto text-slate-300" />
          <p className="text-sm font-medium">No requests matching "{filter}" filter.</p>
          <Link
            to="/farmer/request"
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-bold text-sm text-slate-900">{req.resourceType}</h3>
                        <span className="text-xs text-slate-400 font-mono">#{req.id}</span>
                      </div>
                      <span className="text-xs text-slate-500">
                        Farm: <strong>{req.farm?.name || 'Assigned Farm'}</strong> • Crop:{' '}
                        {req.farm?.crop || 'Crop'} ({req.cropStage})
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      Priority Score: {req.priorityScore?.toFixed(1) || '0.0'}
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-slate-600">
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold uppercase text-slate-400">Requested Window</span>
                    ...
                    <span className="text-[11px] text-slate-500">Duration: {req.durationMinutes} minutes</span>
                  </div>

                  <div className="space-y-1">
                    <span className="text-[10px] font-bold uppercase text-slate-400">Risk Assessment</span>
                    <div>
                      Urgency: <strong className="text-slate-800">{req.urgencyLevel}</strong>
                    </div>
                    <div>
                      Weather Risk: <strong className="text-slate-800">{req.weatherRisk}</strong>
                    </div>
                  </div>

                  <div className="space-y-1">
                    <span className="text-[10px] font-bold uppercase text-slate-400">Assigned Equipment</span>
                    {req.booking ? (
                      <div className="text-emerald-700 font-bold">
                        {req.booking.resource?.name || 'Allocated Unit'}
                      ...
                      <div className="text-amber-600 font-medium italic">
                        {req.status === 'WAITLIST' ? 'Queued in dynamic waitlist' : 'No booking attached'}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs">
                  <span className="text-[11px] text-slate-400">
                    Created: {new Date(req.createdAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}
                  </span>
